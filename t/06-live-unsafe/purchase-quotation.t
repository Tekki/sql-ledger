...
subtest 'Update latest purchase quotation' => sub {
...
subtest 'Add new quotations' => sub {
  for my $quo ($t->config->{request_quotation}{new}->@*) {
    $t->get_ok('Quotation screen', 'oe.pl', action => 'add', type => 'request_quotation')
...
subtest 'Print quotation' => sub {
  ...
  for my ($lang, $templates) ($t->config->{request_quotation}{print}->%*) {
    $t->set_params_ok(
