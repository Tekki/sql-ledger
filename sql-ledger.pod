...
L<instructions from DWS|http://sql-ledger.com/cgi-bin/nav.pl?pagesource/readme.txt&titleREADME>,
...
WLprinter, included in the C<full> branch, is a Java program that is executed
on the client PC and allows to print directly from SQL-Ledger to your local
printers. It is available for printing if you add a printer with command
C<wlprinter> at C<System--Workstations>. The client program is started from
C<Batch--WLprinter>. You will have to add a Java security exception for your
SQL-Ledger server.
...
=item * L<binE<sol>mozillaE<sol>mrp.pl|bin::mozilla::mrp> - Material Requirements Planning
...
=item * L<SL::MRP> - Material requirements planning module
...
=item * L<SL::TagHelpers> - HTML tag helpers
