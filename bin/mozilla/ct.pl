...
for ('', "/$myconfig{dbname}", "/$form->{login}") {
  if (-f "$form->{path}/custom$_/ct.pl") {
    eval { require "$form->{path}/custom$_/ct.pl"; };
  }
}
...
L<bin::mozilla::ct> contains the customer/vendor module.
...
L<bin::mozilla::ct> implements the following functions:
...
=head2 credit_invoice

=head2 credit_note

=head2 customer_pricelist
...
=head2 delete_customers

=head2 delete_vendors

=head2 display_form

=head2 display_pricelist
...
=head2 pricelist_footer

=head2 pricelist_header
