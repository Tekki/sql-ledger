...
sub update {
  ...
  if ($form->{discount_paid}) {
    $form->{discount_exchangerate} = $exchangerate if ($form->{discount_forex} = ($exchangerate = $form->check_exchangerate(\%myconfig, $form->{currency}, $form->{discount_datepaid}, 'sell')));
