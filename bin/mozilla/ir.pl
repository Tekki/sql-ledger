...
|;
  ...
  $form->hide_form(qw(address1 streetname buildingnumber address2 city state zipcode country message email subject cc bcc taxaccounts helpref));
...
sub form_footer {
  ...
  if ($form->{taxaccounts}) {
    $taxincluded = qq|
              ...
              <tr>
                <td align=right>
                <input name=taxincluded class=checkbox type=checkbox value=1 $form->{taxincluded} onChange="doSubmit(document.main)"></td><th align=left>|.$locale->text('Tax Included').qq|</th>
              </tr>
|;
  }
...
|;
  ...
  $form->hide_form(qw(rowcount readonly callback path login _updated));
...
L<bin::mozilla::ir> contains the inventory received module.
...
L<bin::mozilla::ir> implements the following functions:
