...
sub display_row {
  ...
  $column_data{lineitemdetail} = qq|<th class=listheading width=1%><input name="allbox" type=checkbox class=checkbox value="1" $form->{allbox} accesskey="A" title="[A]" onChange="CheckAll(); doSubmit(document.main)"></th>|;
...
# $locale->text('Quotation Date missing!')
  ...
  if ($oldform) {
    ...
    for (1 .. $oldform->{paidaccounts}) {
      $oldform->{"paid_$_"} = $oldform->parse_amount(\%myconfig, $oldform->{"paid_$_"});
    }
...
sub ship_to {
  ...
  %shipto = (
    ...
    buildingnumber => {i => 4,  label => $locale->text('Number')},
...
|;
  ...
  for (qw(name address1 streetname buildingnumber address2 city state zipcode country contact phone fax email recurring)) {
...
L<bin::mozilla::io> contains common routines used in is, ir, oe, ic.
...
F<< bin/mozilla/custom/$form->{login}/io.pl >>,
F<bin/mozilla/custom/io.pl>

=back

=head1 FUNCTIONS

L<bin::mozilla::io> implements the following functions:
...
=head2 shipto_selected

=head2 validate_items
