...
|;
  ...
  if (($rows = $form->numtextrows($form->{description}, 60, 5)) > 1) {
    $description = qq|<textarea name="description" rows=$rows cols=60 wrap=soft accesskey="-" title="$myconfig{dateformat} [-]">$form->{description}</textarea>|;
  } else {
    $description = qq|<input name=description size=60 value="|.$form->quote($form->{description}).qq|" accesskey="-" title="$myconfig{dateformat} [-]">|;
  }
  $description = qq|
              <tr valign=top>
...
|;
  ...
  if ($form->{"select$form->{vc}"}) {
    $vc .= qq|
                <td nowrap><select name=$form->{vc} onChange="doSubmit(document.main)" accesskey="/" title="[/]">|
...
| . $form->hide_form("$form->{vc}number");
    ...
    $vc .= qq|
                <td nowrap><input name=$form->{vc} value="|
      . $form->quote($form->{$form->{vc}})
      . qq|" size=35 accesskey="/" title="[/]">
                $vcref
                </td>
              </tr>
              <tr>
...
sub form_footer {
  ...
  if ($form->{taxaccounts}) {
    $taxincluded = qq|
            ...
            <tr>
              <td align=right>
              <input name=taxincluded class=checkbox type=checkbox value=1 $form->{taxincluded} onChange="doSubmit(document.main)"></td>
  ...
  if (@{ $form->{"all_$form->{vc}"} }) {
    $vc = qq|
         <tr>
           ...
           <td colspan=3><select name=$form->{vc} accesskey="/" title="[/]"><option>\n|;
    ...
    <td>
      <table>
        ...
        <tr>
          ...
          <td colspan=3><input name="description" size=40 accesskey="-" title="[-]"></td>
        </tr>
        <tr>
...
L<bin::mozilla::oe> contains the order entry module,
quotation module.
...
F<< bin/mozilla/custom/$form->{login}/oe.pl >>,
F<< bin/mozilla/custom/oe.pl >>

=back

=head1 FUNCTIONS

L<bin::mozilla::oe> implements the following functions:

=head2 add

=head2 consolidate_orders

=head2 consolidate_orders_to_invoice

=head2 create_backorder

=head2 delete

=head2 display_ship_receive
...
=head2 generate_purchase_orders

=head2 generate_sales_invoices
...
=head2 print_and_save

=head2 print_and_save_as_new
