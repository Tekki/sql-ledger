...
|;
  ...
  $form->hide_form(qw(id type media format printed title closedto locked oldtransdate oldcheckedin oldcheckedout oldpartnumber project oldqty oldnoncharge));
...
|;
  ...
  $form->hide_form(qw(id type media format printed title closedto locked oldtransdate oldpartnumber project));
...
sub list_timecard {
  ...
  @a = qw(type direction oldsort path login sessionid project l_subtotal);
  ...
  @column_index = $form->sort_columns(qw(transdate projectnumber projectdescription id partnumber description allocated 1 2 3 4 5 6 7));
...
|;
    ...
    $column_data{$ref->{weekday}} = "<td align=right>".$form->format_amount(\%myconfig, $ref->{qty}, "", "&nbsp;")."</td>";
