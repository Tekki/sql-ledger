...
sub retrieve_card ($, $myconfig, $form) {
  ...
  if (($form->{id} ||= 0) *= 1) {
    ...
    if ($form->{customer_id}) {
      ...
      SL::PM->price_matrix($pmh, $ref, $form->datetonum($myconfig, $form->{transdate}), 4, $form, $myconfig);
    }
  ...
  SL::JC->jcitems_links($myconfig, $form, $dbh);
...
sub jcitems_links ($, $myconfig, $form, $dbh = undef) {
...
sub jcitems ($, $myconfig, $form) {
  ...
  if (($form->{type} // '') eq 'timecard') {
    $sortorder = 'employee, employeenumber';
  }
  if ($sortorder) {
    $query .= qq| ORDER BY $sortorder,| .$form->sort_order(\@sf);
  } else {
...
=head2 delete
  ...
  SL::JC->delete($myconfig, $form, $spool);
