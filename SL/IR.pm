    ...
    if ($form->{groupprojectnumber} || $form->{grouppartsgroup}) {
      if ($item->[1] ne $sameitem) {
        ...
        if ($ok) {
          ...
          for (qw(taxrates runningnumber number sku serialnumber ordernumber customerponumber bin qty ship unit deliverydate projectnumber projectdescription sell sellprice listprice netprice discount discountrate linetotal itemnotes lineitemdetail package netweight grossweight volume countryorigin hscode barcode lot expires make model)) { push(@{ $form->{$_} }, "") }
...
sub post_invoice ($, $myconfig, $form, $dbh = undef) {
  ...
  if (($form->{id} ||= 0) *= 1) {
    ...
    if ($dbh->selectrow_array($query)) {
      ...
      $query = qq|SELECT aa_id FROM oe
                  WHERE aa_id = $form->{id}|;
      ($form->{aa_id}) = $dbh->selectrow_array($query);
  ...
  for my $i (1 .. $form->{rowcount}) {
    ...
    if ($form->{"qty_$i"}) {
      ...
      my @taxaccounts = split / /, $form->{"taxaccounts_$i"} // '';
  ...
  $query = qq|SELECT sellprice
              ...
              LIMIT 1|;
  my $lth = $dbh->prepare($query) or $form->dberror($query);
  ...
  for my $item (keys %updparts) {
    $ath->execute($item) or $form->dberror;
    ...
    $lth->execute($item) or $form->dberror;
    my ($lastcost) = $lth->fetchrow_array;
    ($lastcost //= 0) *= 1;
    $lth->finish;

    $sth->execute($avgcost, $lastcost, $item) or $form->dberror;
    $sth->finish;
...
sub process_kit ($dbh, $form, $project_id, $i) {
...
sub cogs ($dbh, $form, $id, $totalqty, $project_id, $i) {
...
sub cogs_returns ($dbh, $form, $id, $totalqty, $project_id, $i) {
...
sub retrieve_invoice ($, $myconfig, $form) {
  ...
  if (($form->{id} ||= 0) *= 1) {
    ...
    while (my $ref = $sth->fetchrow_hashref) {
      ...
      # price matrix
      SL::PM->price_matrix($pmh, $ref, undef, $form->{precision}, $form, $myconfig);
...
L<SL::IR> contains the inventory received module.

=head1 FUNCTIONS

L<SL::IR> implements the following functions:

=head2 cogs

  IR::cogs($dbh, $form, $id, $totalqty, $project_id, $i);

=head2 cogs_returns

  IR::cogs_returns($dbh, $form, $id, $totalqty, $project_id, $i);

=head2 delete_invoice

  SL::IR->delete_invoice($myconfig, $form, $spool, $dbh);
...
=head2 item_links
  ...
  SL::IR->item_links($myconfig, $form);

=head2 post_invoice

  SL::IR->post_invoice($myconfig, $form, $dbh);

=head2 process_kit

  IR::process_kit($dbh, $form, $project_id, $i, $decimalplaces);
