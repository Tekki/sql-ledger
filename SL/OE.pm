...
sub transactions ($, $myconfig, $form) {
  ...
  $query = qq|SELECT o.id, o.ordnumber, o.transdate, o.reqdate,
  ...
  if ($form->{type} =~ /^consolidate_/) {
    @d = ();
    for my $ref (@{ $form->{OE} }) { push @d, $ref if $objid{vc}{$ref->{curr}}{$ref->{"$form->{vc}_id"}} > 1 }
...
sub lookup_order ($, $myconfig, $form) {
  ...
  for my $i (1 .. $form->{rowcount}) {
    ...
    if ($form->{"qty_$i"}) {
      ...
      for (0 .. 1) {
        ...
        $taxrate *= $ml;
        my $taxamount = $linetotal * $taxrate / (1 + $taxrate);
        my $taxbase = ($linetotal - $taxamount);
      ...
      if ($form->{"kit_$i"}) {
        my %p = split /[: ]/, $form->{"pricematrix_$i"} // '';
        for (split / /, $form->{"kit_$i"} // '') {
          ...
          for my $n (3 .. $#p) {
            if ($form->{taxaccounts} =~ /$p[$n]/) {
              if ($p[1]) {
                if ($p{0}) {
                  my $r = $fxsellprice/$p{0};
                  my $d = $form->round_amount($p[2] * $r * $form->{"discount_$i"}, $form->{precision});
...
sub delete ($, $myconfig, $form, $spool) {
...
sub retrieve ($, $myconfig, $form) {
  ...
  if (($form->{id} ||= 0) *= 1) {
    ...
    $sth->finish;
    for (qw(printed emailed queued)) { $form->{$_} =~ s/ +$//g if $form->{$_} }
    ...
    while (my $ref = $sth->fetchrow_hashref) {
      ...
      my ($decimalplaces) = ($ref->{sellprice} =~ /\.(\d+)/);
      $decimalplaces = length $decimalplaces // 0;
...
sub order_details ($, $myconfig, $form) {
  ...
  my @sortlist = ();
  my @taxaccounts;
  my $ARAP = $form->{type} eq 'sales_order' ? 'AR' : 'AP';
  ...
  $form->{text_packages} = $c->num2text($form->{packages} // 0);
  ...
  if ($form->{warehouse_id} && $form->{formname} =~ /(pick|packing)_list/) {
    ...
    for my $i (1 .. $form->{rowcount} - 1) {
      $sth->execute($form->{"id_$i"}, $form->{warehouse_id}) or $form->dberror;
            ...
            for my $n (3 .. $#p) {
              if ($form->{taxaccounts} =~ /$p[$n]/) {
                if ($p[1]) {
                  if ($p{0}) {
                    my $r = $sellprice/$p{0};
                    my $d = $form->round_amount($p[2] * $form->{"discount_$i"}/100, $decimalplaces);
                    my $lt = $form->round_amount($p[2] - $d, $form->{precision});
  ...
  my ($paymentaccno) = split /--/, $form->{"${ARAP}_paid_$form->{paidaccounts}"} // '';

  for my $i (1 .. $form->{paidaccounts}) {
    if ($form->{"paid_$i"}) {
      push(@{ $form->{payment} }, $form->{"paid_$i"});
      my ($accno, $description) = split /--/, $form->{"${ARAP}_paid_$i"} // '';
  ...
  if ($form->{paidaccounts} > 1) {
    ...
    $form->{payment_accno} = $form->{"${ARAP}_paid_$i"};
    $form->{payment_accno} =~ s/--.*//;
  }

  $form->{total} = $form->format_amount($myconfig, $form->{ordtotal} - ($form->{paid} // 0), $form->{precision}, 0);
...
sub save_inventory ($, $myconfig, $form) {
  ...
  my $query = qq|SELECT oi.serialnumber, oi.ship
                 FROM orderitems oi
  ...
  $query = qq|SELECT id
              FROM cargo
              WHERE id = ?|;
  $cth = $dbh->prepare($query) or $form->dberror($query);
  ...
  for my $i (1 .. $form->{rowcount} -1) {
    ...
    if ($ship) {
      ...
      $sth->execute($form->{id}, $form->{"orderitems_id_$i"}) or $form->dberror;
      ...
      $cth->execute($form->{"orderitems_id_$i"}) or $form->dberror;
...
sub adj_onhand ($dbh, $form, $ml) {
  ...
  $query = qq|SELECT p.id, p.inventory_accno_id, a.qty
              ...
              JOIN parts p ON (p.id = a.parts_id)
              WHERE a.aid = ?|;
  my $kth = $dbh->prepare($query) or $form->dberror($query);
  my $kref;
  ...
  while (my $ref = $sth->fetchrow_hashref) {
    ...
    if ($ref->{inventory_accno_id} || $ref->{assembly}) {
      ...
      if ($ref->{assembly}) {
        $ath->execute($ref->{parts_id}) or $form->dberror($query);
    ...
    unless ($ref->{inventory_accno_id} || $ref->{income_accno_id} || $ref->{expense_accno_id}) {
      ...
      while (my $kref = $kth->fetchrow_hashref) {
...
sub add_items_required ($, $dbh, $form, $id, $parts_id, $required, $assembly, $orderitemsid, $ndx) {
...
sub generate_orders ($, $myconfig, $form) {
  ...
  for (my $i = 1; $i <= $form->{rowcount}; $i++) {
    ...
    if ($form->{"qty_$i"}) {
      my ($vendor, $vendor_id) = split /--/, $form->{"vendor_$i"} // '';
...
sub consolidate_orders ($, $myconfig, $form) {
  ...
  $query = qq|INSERT INTO reference (
              ...
              (?,?,?,?,?)|;
  my $irth = $dbh->prepare($query) or $form->dberror($query);
  ...
  for my $curr (keys %{ $oe{oe} }) {

    for my $id (sort { $oe{oe}{$curr}{$a}->{ndx} <=> $oe{oe}{$curr}{$b}->{ndx} } keys %{ $oe{oe}{$curr} }) {

      # retrieve order
      my $vc_id = $oe{oe}{$curr}{$id}->{"$form->{vc}_id"};
  ...
  for my $curr (keys %{ $oe{orders} }) {
    ...
    for my $vc_id (sort { $a <=> $b } keys %{ $oe{orders}{$curr} }) {
      # the orders
      @orderitems = ();
      ...
      for my $id (@{ $oe{orders}{$curr}{$vc_id} }) {
        ...
        for my $item (@{ $oe{orderitems}{$curr}{$id} }) {
...
L<SL::OE> contains the order entry module,
quotation.

=head1 FUNCTIONS

L<SL::OE> implements the following functions:

=head2 add_items_required

  SL::OE->add_items_required($dbh, $form, $id, $parts_id, $required, $assembly, $orderitemsid, $ndx);

=head2 adj_inventory

  OE::adj_inventory($dbh, $form, $i);

=head2 adj_onhand

  OE::adj_onhand($dbh, $form, $ml);

=head2 assembly_details

  OE::assembly_details($myconfig, $form, $dbh, $id, $qty, $kit);

=head2 consolidate_orders

  SL::OE->consolidate_orders($myconfig, $form);

=head2 delete

  SL::OE->delete($myconfig, $form, $spool);

=head2 generate_orders

  SL::OE->generate_orders($myconfig, $form);
...
=head2 get_warehouses
  ...
  SL::OE->get_warehouses($myconfig, $form, $dbh);

=head2 lookup_order

  SL::OE->lookup_order($myconfig, $form);
...
=head2 project_description
  ...
  SL::OE->project_description($dbh, $id);
...
=head2 save_inventory

  SL::OE->save_inventory($myconfig, $form);
